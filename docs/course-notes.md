...
> **Status:** 🟡 Core done — per-request trace (trace_id + compile/execute spans + latency + attrs) persisted to `_traces`, exposed via `/traces` + receipt; plus `_audit`. Missing: full OTel SDK, cross-process span propagation, bytes/credits cost. · [→ DESIGN §16](DESIGN.md#16-status--remaining-work-honest-vs-this-designs-full-depth)
