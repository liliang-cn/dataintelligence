...
> **Status:** ✅ Built — **hybrid retrieval** (cortexdb BM25 ⊕ dense embeddings via DashScope `text-embedding-v4`, in-process cosine fusion) + **few-shot exemplar bank** (`models/exemplars.yaml`: embedding-retrieved, shape-deduped, injected into the prompt; `di exemplar` promotes fixed misses) + clarify gate (`grounding/`, `di ask`). + **cross-encoder rerank** (`grounding/rerank.go`: a joint (question, metric) scorer — name/synonym coverage + description Jaccard, blended with retrieval — reorders the top-K for precision@1; pluggable for an LLM reranker) + **cost benchmark** (`di nleval` reports end-to-end p50/p95/max latency). Verified: rerank keeps the gate at 100%; cost p50≈8ms. · [→ DESIGN §16](DESIGN.md#16-status--remaining-work-honest-vs-this-designs-full-depth)
...
> **Status:** ✅ Built — read-only sessions + timeout + row cap + write-approval (flow) + **pre-execution byte ceiling** (`warehouse.Estimate`/`GuardCost` via `EXPLAIN`; `DI_MAX_SCAN_BYTES` refuses an over-budget query before a row is read — verified) + estimated cost recorded on the trace + **per-tenant spend ledger** (`governance/spend.go`, `di spend`): cumulative planner-estimated bytes per tenant in `_spend`, with a `TenantBudgetBytes` cap that refuses a tenant's queries once over budget — verified (2 queries = 25,600 B > 20,000 B → refused). · [→ DESIGN §16](DESIGN.md#16-status--remaining-work-honest-vs-this-designs-full-depth)
...
> **Status:** ✅ 8-dim rubric, all dimensions strong: **Semantic Model** (grain-to-date + additivity), **Grounding** (hybrid + few-shot + cross-encoder rerank + cost benchmark), **Evaluation** (47-case set, 100% offline + CI gate), **Governance** (RLS/masking/k-anon + threat-as-code), **MCP-Security** (real OIDC/OBO + `di pentest` 9-attack gate), **Autonomy** (write-back + byte ceiling + per-tenant spend ledger), **Drift** (version registry + canary + lineage invalidation + auto-rollback watcher), **Observability** (real OTel: span tree + cost attrs + W3C cross-process propagation). Remaining is breadth, not capability: extra warehouse dialects (Snowflake/Databricks), bridge/role-playing dims. · [→ DESIGN §16](DESIGN.md#16-status--remaining-work-honest-vs-this-designs-full-depth)
