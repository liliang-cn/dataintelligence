...
| §8 Guardrails | 🟡 | timeout + row cap, write approval (flow). missing: spend/credit caps, read-only-by-default grants |
...
| Connectors/Sinks | 🟡 | sources: CSV + **Postgres-CDC** (real, poll-based, `di cdc`). sinks: log/json/table/alert + **Elasticsearch** (real, `_bulk` into Dockerized ES, verified) + Snowflake (dry-run, no account). missing: webhook source, CRM clients, live Snowflake driver |
...
8. ~~Connectors/Sinks breadth~~ — **DONE**: Postgres-CDC source (`di cdc`) + real Elasticsearch sink (`-to es:`, Dockerized) + Snowflake dry-run sink. (left: webhook source, CRM, live Snowflake)
